...
export function insertPipeline(columnNames: string[], tableName: string, columnValues: any[]) {
    ...
    if (columnNames.length > 1) {
        queryStr = `INSERT INTO spec.${tableName}(${columnNames[0]}, ${columnNames[1]}) VALUES ('${columnValues[0]}',${columnValues[1]}) RETURNING *`;
    }
    else {
        queryStr = `INSERT INTO spec.${tableName}(${columnNames[0]}) VALUES ('${columnValues[0]}') RETURNING *`;
